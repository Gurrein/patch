function preSplit(date)
    ...
    for i= 3:length(folders)
        ...
        numROIs = str2double(nameParts{3}); % 3rd index corresponds to number of strains
        for s = 1:numROIs
        ...
        ROIs =  struct(); % contains regions to be cropped, one for each strain
        c = 1;
        figure;
        imshow(vidFrame, []);
        hold on;    
        for s = 1:numROIs % crops original video into relevant region for each strain
            [x, y] = getROI(strains{s},colors{c});
            ROIs(s).x = x; % top left corner of relevant region
            ROIs(s).y = y; % top right corner of relevant region
...
function [x, y] = getROI(strain, color)
